...
public:
    ...
    Individu(const ConfigType& config, RNG& rng)
        ...
        // Initialiser la probabilité d'AUTO-ADAPTATION (séparément)
     ...
     */
    Individu() : m_config(nullptr), m_num_vectors(0), m_dimension(0) {}
    ...
    void invalidate_fitness() {
        ...
        // Optionnel mais recommandé : réinitialiser la fitness à la pire valeur
        if (m_config) {
...
private:
    ...
    void mutate_proba(RNG& rng) {
        ...
        // Le code ici est inchangé par rapport à votre logique d'auto-adaptation.
    ...
    void mutate_vectors(RNG& rng) {
        ...
        if (m_config->enable_auto_adaptation) {
            // --- BLOC OPTIMISÉ POUR L'ADAPTATION (Taux variable) ---
            for (size_t i = 0; i < m_num_vectors; i++) {
            ...
            // --- BLOC OPTIMISÉ POUR LE TAUX FIXE (Taux constant) ---
            // Calcule le taux fixe UNE seule fois (ou le lit depuis la configuration)
            Real fixed_rate = m_config->get_fixed_proba();

            for (size_t i = 0; i < m_num_vectors; i++) {
                // Le taux 'fixed_rate' est constant pour tous les vecteurs 'i'
    ...
    void mutate_single_array(RNG& rng, Real rate, std::array<Integer, N>& target_array, size_t array_size) {
        // La boucle utilise maintenant la taille DYNAMIQUE (array_size)
